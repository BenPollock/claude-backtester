...
# ---------------------------------------------------------------------------
# Scenario 6: DataFrame with only 1 row
# ---------------------------------------------------------------------------

class TestSingleRowData:
    """Single day of data within a year-long backtest range."""

    def test_single_row_no_crash(self):
        single = make_df(start="2020-06-15", days=1, start_price=100.0)
        bench = make_df(start="2020-01-02", days=252)

        src = MockDataSource()
        src.add("SINGLE", single)
        src.add("SPY", bench)

        config = _make_config(tickers=["SINGLE"], strategy="always_buy_test",
                              params={}, start="2020-01-01", end="2020-12-31")
        ...
        try:
            ...
            pytest.skip(f"Single-row data raises: {type(e).__name__}: {e}")


# ---------------------------------------------------------------------------
# Scenario 7: Non-business-day dates in index
# ---------------------------------------------------------------------------

class TestNonBusinessDayDates:
    """DataFrame with Saturday/Sunday dates in the index."""

    def test_weekend_dates_no_crash(self):
        ...
        config = _make_config(tickers=["WEEKEND"], strategy="always_buy_test",
                              params={}, start="2020-01-01", end="2020-12-31")
        ...
        try:
            ...
            pytest.skip(f"Weekend dates raise: {type(e).__name__}: {e}")
...
# ---------------------------------------------------------------------------
# Scenario 9: Missing Volume column
# ---------------------------------------------------------------------------

class TestMissingVolumeColumn:
    """DataFrame with OHLC only, no Volume."""

    def test_no_volume_column(self):
        df = make_df(start="2020-01-02", days=252)
        ...
        try:
            ...
            pytest.skip(f"Missing Volume column raises: {type(e).__name__}: {e}")
...
# ---------------------------------------------------------------------------
# Additional edge case: Duplicate dates in index
# ---------------------------------------------------------------------------

class TestDuplicateDates:
    """DataFrame with duplicate dates in the index."""
    ...
    def test_duplicate_dates(self):
        ...
        config = _make_config(tickers=["DUP"], strategy="always_buy_test",
                              params={}, start="2020-01-01", end="2020-12-31")
        ...
        try:
            ...
            pytest.skip(f"Duplicate dates raise: {type(e).__name__}: {e}")


# ---------------------------------------------------------------------------
# Additional edge case: NaN prices in data
# ---------------------------------------------------------------------------

class TestNaNPrices:
    """DataFrame with some NaN values in price columns."""
    ...
    def test_nan_in_close(self):
        ...
        config = _make_config(tickers=["NANNY"], strategy="always_buy_test",
                              params={}, start="2020-01-01", end="2020-12-31")
        ...
        try:
            ...
            equity = _equity_values(result)
            nan_count = sum(1 for v in equity if not np.isfinite(v))
            assert nan_count < len(equity) * 0.5, \
                f"Too many NaN equity values: {nan_count}/{len(equity)}"
        except Exception as e:
            pytest.skip(f"NaN prices raise: {type(e).__name__}: {e}")


# ---------------------------------------------------------------------------
# Additional edge case: Negative prices
# ---------------------------------------------------------------------------

class TestNegativePrices:
    """DataFrame with negative prices (e.g. bad data)."""

    def test_negative_prices(self):
        df = make_df(start="2020-01-02", days=252, start_price=-10.0)
        bench = make_df(start="2020-01-02", days=252)
        ...
        config = _make_config(tickers=["NEG"], strategy="always_buy_test",
                              params={}, start="2020-01-01", end="2020-12-31")
        ...
        try:
            ...
            pytest.skip(f"Negative prices raise: {type(e).__name__}: {e}")
