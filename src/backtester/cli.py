...
_common_options = [
    ...
    # EDGAR integration
    click.option("--use-edgar", is_flag=True, default=False,
                 help="Fetch data from SEC EDGAR (financials, insider, events)"),
    click.option("--edgar-user-agent", default=None,
                 help="User-Agent for SEC EDGAR (email required by SEC)"),
    click.option("--no-edgar-financials", is_flag=True, default=False,
                 help="Disable EDGAR financial statements"),
    click.option("--no-edgar-insider", is_flag=True, default=False,
                 help="Disable EDGAR insider trading data"),
    click.option("--edgar-institutional", is_flag=True, default=False,
                 help="Enable EDGAR 13F institutional holdings (slower)"),
    click.option("--no-edgar-events", is_flag=True, default=False,
                 help="Disable EDGAR 8-K event data"),
    click.option("--edgar-max-filings", default=50, type=int,
                 help="Max filings per EDGAR source (Form 4, 8-K, 13F; default: 50)"),
    # Alternative data sources
    click.option("--use-vix", is_flag=True, default=False,
                 help="Enable VIX term structure data"),
    click.option("--use-intermarket", is_flag=True, default=False,
                 help="Enable cross-asset intermarket signals (copper/gold, dollar)"),
    click.option("--use-fred", is_flag=True, default=False,
                 help="Enable FRED macro regime data (requires FRED_API_KEY)"),
    click.option("--fred-api-key", default=None,
                 help="FRED API key (or set FRED_API_KEY env var)"),
    click.option("--fred-regime-mode", type=click.Choice(["supplement", "replace"]),
                 default="supplement",
                 help="How FRED regime interacts with SMA regime filter"),
    click.option("--use-yield-curve", is_flag=True, default=False,
                 help="Enable Treasury yield curve data from FRED"),
    click.option("--use-pcr", is_flag=True, default=False,
                 help="Enable CBOE equity put-call ratio"),
    click.option("--use-analyst", is_flag=True, default=False,
                 help="Enable analyst earnings revision data"),
...
def run(strategy, tickers, market, universe, benchmark, start, end, cash, max_positions,
        ...
        use_edgar, edgar_user_agent, no_edgar_financials, no_edgar_insider,
        edgar_institutional, no_edgar_events, edgar_max_filings,
        use_vix, use_intermarket, use_fred, fred_api_key, fred_regime_mode,
        use_yield_curve, use_pcr, use_analyst,
    ...
    config = _build_config(
        ...
        use_edgar=use_edgar, edgar_user_agent=edgar_user_agent,
        no_edgar_financials=no_edgar_financials, no_edgar_insider=no_edgar_insider,
        edgar_institutional=edgar_institutional, no_edgar_events=no_edgar_events,
        edgar_max_filings=edgar_max_filings,
        use_vix=use_vix, use_intermarket=use_intermarket,
        use_fred=use_fred, fred_api_key=fred_api_key,
        fred_regime_mode=fred_regime_mode, use_yield_curve=use_yield_curve,
        use_pcr=use_pcr, use_analyst=use_analyst,
...
def _build_config(strategy, tickers, market, universe, benchmark, start, end,
                   ...
                   use_edgar=False, edgar_user_agent=None,
                   no_edgar_financials=False, no_edgar_insider=False,
                   edgar_institutional=False, no_edgar_events=False,
                   edgar_max_filings=50,
                   use_vix=False, use_intermarket=False,
                   use_fred=False, fred_api_key=None,
                   fred_regime_mode="supplement", use_yield_curve=False,
                   use_pcr=False, use_analyst=False,
                   fill_price="open",
    ...
    return BacktestConfig(
        ...
        use_edgar=use_edgar,
        edgar_user_agent=edgar_user_agent or "claude-backtester research@example.com",
        edgar_financials=not no_edgar_financials if use_edgar else True,
        edgar_insider=not no_edgar_insider if use_edgar else True,
        edgar_institutional=edgar_institutional,
        edgar_events=not no_edgar_events if use_edgar else True,
        edgar_max_filings=edgar_max_filings,
        use_vix=use_vix,
        ...
        use_yield_curve=use_yield_curve,
        use_pcr=use_pcr,
